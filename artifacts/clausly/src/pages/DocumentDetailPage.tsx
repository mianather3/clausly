...
export default function DocumentDetailPage({ id }: { id: string }) {
  ...
  const fetchSigStatus = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/documents/${docId}/signature-status`, { headers: { Authorization: `Bearer ${token}` } });
      setSigStatus(r.ok ? await r.json() : null);
    } catch { setSigStatus(null); }
  }, [docId, getToken]);
  ...
  const handleDownloadDocx = async () => {
    ...
    try {
      ...
      toast({ title: "Failed to download.", description: err instanceof Error ? err.message : undefined, variant: "destructive" });
  ...
  const sigBadge = sigStatus === undefined ? null : sigStatus === null ? null : (
    sigStatus.status === "signed" ? (
      <Badge className="bg-green-400/10 text-green-400 border-green-400/20 gap-1">
        <CheckCircle className="h-3 w-3" />Signed by {sigStatus.recipientName || sigStatus.recipientEmail}
      </Badge>
    ) : (
      <Badge className="bg-yellow-400/10 text-yellow-400 border-yellow-400/20 gap-1">
        <Clock className="h-3 w-3" />Pending Signature · {sigStatus.recipientEmail}
      </Badge>
    )
  );
          ...
          <Card className="bg-card border-border w-full max-w-md mx-4 shadow-2xl">
            <CardHeader className="flex flex-row items-start justify-between pb-3">
              <div>
                ...
                <p className="text-muted-foreground text-sm mt-1">The recipient will receive an email with a link to sign.</p>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <label className={labelClass}>Recipient Name <span className="text-white/30 normal-case">(optional)</span></label>
                <input
                  className={fieldClass}
              ...
              <p className="text-xs text-muted-foreground">Electronic signatures collected through Clausly are legally binding under the E-SIGN Act and UETA.</p>
              <div className="flex gap-3">
                ...
                <Button variant="outline" onClick={() => setShowSigModal(false)} className="border-border text-white hover:bg-secondary">Cancel</Button>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <Card className="bg-card border-border">
        <CardHeader className="flex flex-row items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex flex-wrap gap-2 flex-shrink-0">
            ...
            {sigStatus?.status !== "signed" && (
              <Button onClick={() => setShowSigModal(true)} variant="outline" size="sm" className="border-primary/40 text-primary hover:bg-primary/10 gap-2">
        ...
        <CardContent>
          ...
          <div className="space-y-4 font-serif text-sm text-white leading-relaxed max-h-[70vh] overflow-y-auto pr-2">
            {paragraphs.map((paragraph, i) => (
              <div
                key={i}
                onClick={() => handleClauseClick(paragraph)}
                className={`cursor-pointer rounded-lg px-3 py-2 transition-all border-l-2 ${
                  selectedClause === paragraph
                    ? "bg-primary/10 border-primary"
                    : "border-transparent hover:bg-black/20 hover:border-primary/30"
                }`}
              >
